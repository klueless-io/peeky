...
RSpec.describe Peeky::ParameterInfo do
  ...
  describe '#from_parameter' do
    ...
    context 'with attribute writer signature' do
      ...
      it { is_expected.to have_attributes(name: '', type: :param_required, signature_format: '', minimal_call_format: "''", default_value: nil) }
    end

    context 'with required positional parameter - simple(aaa)' do
      let(:param) { [:req, 'aaa'] }

      it { is_expected.to have_attributes(name: 'aaa', type: :param_required, signature_format: 'aaa', minimal_call_format: "'aaa'", default_value: nil) }
    end

    context 'with optional positioned parameter - simple(aaa = nil)' do
      let(:param) { [:opt, 'aaa'] }

      it { is_expected.to have_attributes(name: 'aaa', type: :param_optional, signature_format: 'aaa = nil', minimal_call_format: '', default_value: nil) }
    end

    context 'with splat positioned parameters - simple(*aaa)' do
      let(:param) { [:rest, 'aaa'] }

      it { is_expected.to have_attributes(name: 'aaa', type: :splat, signature_format: '*aaa', minimal_call_format: '', default_value: nil) }
    end

    context "with required keyed/named parameter - simple(aaa: 'aaa')" do
      let(:param) { [:keyreq, 'aaa'] }

      it { is_expected.to have_attributes(name: 'aaa', type: :key_required, signature_format: 'aaa:', minimal_call_format: "aaa: 'aaa'", default_value: nil) }
    end
    ...
    context 'with optional keyed/named parameter - simple(&aaa)' do
      ...
      it { is_expected.to have_attributes(name: 'aaa', type: :block, signature_format: '&aaa', minimal_call_format: '', default_value: nil) }
    end
  end
